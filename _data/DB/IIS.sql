...
CREATE TABLE IF NOT EXISTS `pe000404db`.`Surovina` (
  ...
  `nakupnaCena` FLOAT(7,2) NOT NULL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pe000404db`.`vObjednavka` (`id` INT, `termin` INT, `suma` INT, `vybavene` INT, `vodic` INT, `podal` INT, `adresa` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pe000404db`.`vObjednavkaFull` (`id` INT, `termin` INT, `suma` INT, `vybavene` INT, `vodic` INT, `podal` INT, `adresa` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pe000404db`.`vObjednavkaOblast` (`id` INT, `termin` INT, `suma` INT, `vybavene` INT, `vodic` INT, `podal` INT, `AdresaId` INT, `OblastRozvozuId` INT, `nazov` INT, `spravuje` INT, `adresa` INT, `clen` INT);
...
USE `pe000404db`;
DROP procedure IF EXISTS `pe000404db`.`before_delete`;

DELIMITER $$
USE `pe000404db`$$
CREATE PROCEDURE `before_delete` (vid CHAR(36))
BEGIN
	UPDATE Stav SET Stav.deleted = 1 WHERE Stav.id = vid;
END
$$
...
USE `pe000404db`;
CREATE  OR REPLACE VIEW `vObjednavkaOblast` AS SELECT o.id, o.termin, o.suma, o.vybavene, o.vodic, o.podal, ora.id as AdresaId, ora.OblastRozvozuId, ora.nazov, ora.spravuje, ora.adresa, ora.clen FROM vObjednavka AS o LEFT JOIN vOblastRozvozuAdresa AS ora ON ora.id = o.adresa;
