...
CREATE TABLE IF NOT EXISTS `pe000404db`.`Clen` (
  ...
  `evidovany` TINYINT(1) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `pe000404db`.`Pecivo` (
  ...
  `povolene` TINYINT(1) NOT NULL DEFAULT 1,
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pe000404db`.`vPecivo` (`id` INT, `nazov` INT, `popis` INT, `cena` INT, `povolene` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `pe000404db`.`vPecivoFull` (`id` INT, `nazov` INT, `popis` INT, `cena` INT, `povolene` INT);
...
USE `pe000404db`;
CREATE  OR REPLACE VIEW `vVodicAdresa` AS SELECT c.*, ora.id as AdresaId, ora.OblastRozvozuId, ora.nazov, ora.adresa, ora.clen FROM vOblastRozvozuAdresa AS ora RIGHT JOIN vClen as c ON c.id = ora.spravuje WHERE ora.id is not NULL;
...
USE `pe000404db`;
CREATE  OR REPLACE VIEW `vPecivoSurovina` AS SELECT p.*, r.mnozstvo, s.nazov as SurovinaNazov, s.nakupnaCena, s.naSklade FROM vPecivo AS p LEFT JOIN vRecept AS r ON r.pecivo = p.id LEFT JOIN vSurovinaFull AS s ON s.id = r.surovina;
